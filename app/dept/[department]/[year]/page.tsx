...
export default function YearSemestersPage() {
  ...
  // Sample semesters - would be dynamic based on department and year
  const semesters = [
    {
      name: "1st Semester",
      slug: "1st-sem",
      ...
      <div className="min-h-screen bg-gray-50">
        <div className="container mx-auto px-4 py-8 max-w-6xl">
          ...
          <div className="mb-8">
            ...
            <p className="text-gray-600">Select a semester to view subjects and study materials</p>
          </div>

          {/* Semesters Grid */}
          <div className="grid md:grid-cols-2 gap-6 max-w-4xl mx-auto">
            {semesters.map((semester, index) => (
              <Link key={index} href={`/dept/${department}/${year}/${semester.slug}`}>
                <Card className="hover:shadow-lg transition-all duration-300 hover:-translate-y-1 cursor-pointer group h-full">
                  <CardHeader className="pb-4">
                    ...
                    <CardTitle className="text-xl leading-tight">{semester.name}</CardTitle>
                    <CardDescription className="text-sm font-medium text-primary">
                      {semester.subjectCount} Subjects
                    </CardDescription>
                  </CardHeader>
                  <CardContent>
                    <CardDescription className="leading-relaxed">{semester.description}</CardDescription>
                  </CardContent>
                </Card>
              </Link>
            ))}
